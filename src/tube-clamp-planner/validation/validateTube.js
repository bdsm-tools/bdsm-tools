...
export default function validateTube(tube, joinFrom) {
    ...
    if (tube.startConnection && joinFrom === 'end') {
        throw 'You cannot have a "startConnection" on a tube that is already being join using an end.'
        + 'Use "endConnection" if free instead.'
    }

    tube.middleConnections?.forEach(connection => validateTypeDefinition(
