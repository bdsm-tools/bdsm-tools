...
export default function CrossoverEditor({ node, getNode, setNode, setChainNode, addChainNode, connection, onDeselect }) {

  const parent = connection.parent ? getNode(connection.parent) : undefined;
  const middle = connection.children.middle[0] ? getNode(connection.children.middle[0]) : undefined;
    ...
    <>
      ...
      >
        <Descriptions.Item label='Connection 1 Position on Tube'>
          <PositionEditorInput node={node} setNode={setNode} max={parent?.node.length} />
        </Descriptions.Item>
        {middle && (
          <Descriptions.Item label='Connection 2 Position on Tube'>
            <PositionEditorInput node={middle.node} setNode={setChainNode(middle.id)} max={middle?.node.length} />
          </Descriptions.Item>
        )}
