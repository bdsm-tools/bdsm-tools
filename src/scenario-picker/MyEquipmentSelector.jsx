...
export default function MyEquipmentSelector({ data }) {
  ...
  React.useEffect(
    () => void Cookies.set('my-equipment', myEquipment.join('|')),
    [myEquipment],
  );
  React.useEffect(
    () => void Cookies.set('missing-equipment', missingEquipment.join('|')),
    [missingEquipment],
  );
  ...
  const equipment = [
    ...new Set(data.flatMap((scene) => scene.requiredEquipment)),
    ...
    <>
      ...
      >
        <Typography.Paragraph>
          Let us know what equipment you have so we can tailor the scenarios to
          what you have.
        </Typography.Paragraph>
        <List
          header='My Equipment'
