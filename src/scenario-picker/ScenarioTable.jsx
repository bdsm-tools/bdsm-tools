...
const Tags = (values = []) => (
  <TagsComponent values={values} colourFunction={participantColourFunction} />
);
...
const columns = [
  {
    ...
    key: 'name',
    render: (scene) => (
      <Link to={`/tools/bdsm-scenarios/${scene.path}`}>{scene.name}</Link>
    ),
  },
  {
    title: 'Description',
...
export default function ScenarioTable({ data = scenarioData }) {
  ...
  const applyCompatibilityFilter =
    compatibilityFilterFunction(compatibilityFilter);

  const filteredDataSource = dataSource
    .filter((scene) => (equipmentFilter ? applyEquipmentFilter(scene) : true))
    .filter((scene) =>
      participantFilter ? applyParticipantFilter(scene) : true,
    )
    .filter((scene) =>
      compatibilityFilter ? applyCompatibilityFilter(scene) : true,
    );
    ...
    <>
      <div style={{ marginBottom: 20 }}>
        ...
        <div className='flex' style={{ alignItems: 'center', marginTop: 10 }}>
          ...
          >
            {[...new Set(data.flatMap((scene) => scene.participants))].map(
              (participant) => (
          ...
          <Typography.Text style={{ width: 50, textAlign: 'center' }}>
            for
          </Typography.Text>
          <Select
      ...
      <Table
        ...
        footer={() => (
          <Typography.Text>
            {filteredDataSource.length} out of {dataSource.length} scenarios
