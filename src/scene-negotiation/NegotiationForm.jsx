...
function NegotiationForm({ template }) {
  ...
  const readOnlyCells = [...ReadOnlyCells];
  const editableCells = [...vanillaCells, ...AntdCells, ...CustomCells];
  if (!template) {
    return null;
  }
    ...
    >
      <div id='jsonforms-container' className='fillspace'>
        <JsonForms
          ...
          renderers={[...vanillaRenderers, ...AntdRenderers]}
      ...
      <Modal open={submit} onCancel={() => setSubmit(false)} footer={null}>
        <ShareForm data={data} errors={errors} template={template.id} />
      </Modal>
    </div>
  );
}
