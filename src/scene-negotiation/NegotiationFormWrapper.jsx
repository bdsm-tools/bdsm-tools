...
const NegotiationForm = React.lazy(
  () =>
    import(
      /* webpackChunkName: "NegotiationForm", webpackPrefetch: true */ './NegotiationForm'
    ),
);
...
export default function NegotiationFormWrapper() {
  ...
  const [template] = (templates || []).filter(
    ({ title }) => decodeURIComponent(type) === title,
  );

  if (type === '__testing__') {
    console.log('Testing Template:', testTemplate);
    return <NegotiationForm template={testTemplate} />;
  }
  if (template) {
    return <NegotiationForm template={template} />;
  }
  return (
    <Result
      status={404}
