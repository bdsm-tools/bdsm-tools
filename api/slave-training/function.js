...
async function updateStats(update) {
  return await mongo('stats', (collection) => {
    collection.updateOne({ _id: 'slave-training' }, update, { upsert: true });
  });
}
  ...
  return {
    async getTask(bodyPart) {
      ...
      if (
        ...
        req.session.slaveTask.getTask.queryCount === 0
      ) {
        res
          .status(400)
          .send('No tasks match the equipment / body parts selected');
      } else {
        const [result] = await mongo(
          'slave-training-tasks',
          async (collection) => {
            if (req.session.slaveTask.getTask.queryHash !== queryHash) {
              req.session.slaveTask.getTask.queryHash = queryHash;
              req.session.slaveTask.getTask.queryCount =
            ...
            const taskNumber = random(
              0,
              req.session.slaveTask.getTask.queryCount,
            );

            return await collection
              .find(filter, { sort: { added: 1 }, limit: 1, skip: taskNumber })
              .toArray();
          },
        );
    ...
    async getRandomTask() {
      ...
      if (
        ...
        req.session.slaveTask.getRandomTask.queryCount === 0
      ) {
        res
          .status(400)
          .send('No tasks match the equipment / body parts selected');
        ...
        const [result] = await mongo(
          ...
          async (collection) => {
            if (req.session.slaveTask.getRandomTask.queryHash !== queryHash) {
              req.session.slaveTask.getRandomTask.queryHash = queryHash;
              req.session.slaveTask.getRandomTask.queryCount =
                await collection.countDocuments(filter);
            }
    ...
    async getDailyTask() {
      ...
      const dateHash = hash(
        moment()
          .utcOffset(Number(req.cookies.timezoneOffset || 0))
          .format('YYYY-MM-DD'),
      );
      ...
      if (req.headers['If-None-Match'] === etag) {
        ...
        if (
          ...
          req.session.slaveTask.getDailyTask.queryCount === 0
        ) {
          res
            .status(400)
            .send('No tasks match the equipment / body parts selected');
          ...
          const [result] = await mongo(
            ...
            async (collection) => {
              if (req.session.slaveTask.getDailyTask.queryHash !== queryHash) {
                req.session.slaveTask.getDailyTask.queryHash = queryHash;
                req.session.slaveTask.getDailyTask.queryCount =
                  await collection.countDocuments(filter);
              }
...
const getStats = async (req, res) =>
  res.status(200).json({
    ...
    completedTasks: (req.session?.slaveTask?.completedTasks ?? []).filter(
      ({ timestamp }) => moment(timestamp).isSame(moment(), 'day'),
    ),
    failedTasks: (req.session?.slaveTask?.failedTasks ?? []).filter(
      ({ timestamp }) => moment(timestamp).isSame(moment(), 'day'),
    ),
  });
...
const completeTask = (isCompleted) => async (req, res) => {
  ...
  if (!isCompleted) {
    ...
    if (
      ...
      await updateStats({
        $max: { dailyStreakHighScore: req.session.slaveTask.stats.dailyStreak },
      });
    } else {
      req.session.slaveTask.stats.dailyStreak = 0;
    }

    req.session.slaveTask.stats.points +=
      100 * (bonus === 'true' ? 2 : 1) * (daily === 'true' ? 5 : 1);
  ...
  const taskStats = await mongo('stats', (collection) => {
    ...
    const successRate =
      successes + failures === 0
        ? -1
        : (successes / (successes + failures)).toFixed(2);
    const successRateWithBonus =
      successesWithBonus + failuresWithBonus === 0
        ? -1
        : (
            successesWithBonus /
            (successesWithBonus + failuresWithBonus)
          ).toFixed(2);
